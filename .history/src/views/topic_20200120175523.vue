              ...
              <section class="user">
                ...
                <div class="info">
                  ...
                  <span class="cr">
                    <span
                      class="iconfont icon"
                      :class="{'uped':isUps(repliy.ups)}"
                      @click="upReply(repliy)"
                    >&#xe608;</span>
                    {{repliy.ups.length}}
                    <span class="iconfont icon" @click="addReply(reply.id)">&#xe609;</span>
                  </span>
                </div>
              </section>
              <div class="reply_content" v-html="repliy.content"></div>
              <nv-top></nv-top>
              <nv-reply v-if="userInfo.userId" :topic="topic" :topicId="topic.id"></nv-reply>
              <!---->
            </li>
          </ul>
        </section>
