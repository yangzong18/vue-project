    ...
    methods: {
      ...
      showMenu: function (num) {
        ...
        this.$store.dispatch('notifyActionSheet', {
          menus: {
            'title.noop': this.cd.songlist[num].songname + '<br/><span style="color:#666;font-size:12px;">' + this.getSingerStr(this.cd.songlist[num].nickname) + '</span>',
            playAsNext: '下一首播放',
          ...
          handler: {
            ...
            ['playAsNext'](){
              that.$store.commit('addToPlayListAsNextPlay', {
                ...
                albumid:that.cd.songlist[that.menuedIndex].nickname,
              })
            },
            ['addToPlayList'](){
