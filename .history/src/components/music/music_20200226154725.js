...
const musicApi = {
    ...
    scrollLyric (time, that) {
        ...
        console.log(document.getElementsByClassName('dw-react-web-lyric-comp-content-list').innerHTML)

        try {
            // this.scrollAnimate(that.$refs.lrcWrapper, i * document.getElementsByClassName('lrc-item')[0].offsetHeight)
            document.getElementsByClassName('dw-react-web-lyric-comp-content-list').innerHTML = this.lyric[time]
        } catch (e) {
            return
        }
    },
