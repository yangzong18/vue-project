        ...
        <section class="user">
          ...
          <div class="info">
              <span class="cl">
                  ...
                  <span class="name" v-if="item.type==='reply'">回复了你的话题</span>
              </span>
              <span class="cr">
                  <span class="name" v-text="getLastTimeStr(item.reply.creat_at,true)"></span>
              </span>
          </div>
        </section>
        <div class="reply_content" v-html="item.reply.content"></div>
        <router-link :to="{name:'topic',params:{id:item.topic.id}}"></router-link>
      </div>
