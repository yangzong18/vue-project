<template>
    <div class="goods">
        <div class="menu-wrapper" ref="menuWrapper">
            <ul>
                <li v-for="(item,index) in  goods" :key="index" :class="{'current':current===index}" class="menu-item" @click="selectMenu(index,$event)">
        ...
        currentIndex() { // 计算当前右侧的距离，决定决定左侧哪一个分类高亮（选中的意思）
            for (let i = 0; i < this.listHeight.length; i++) {
                ...
                if (!height2 || (this.scrollY >= height1 && this.scrollY < height2)) {
                    this.current = i; // 返回当前索引
                    return 
                }
            }
            this.current = 0; // 返回当前索引
            return
        },
    },
    mounted(){
