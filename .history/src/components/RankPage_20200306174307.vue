  ...
  methods:{
    ...
    showMenu(index){
      ...
      this.$store.dispatch('notifyActionSheet',{
          menus:{
              'title.noop':`${this.topListData.songlist[index].data.songorig}<br/><span style="color:#666;font-size:12px;">${this.getSingerStr(this.topListData.songlist[num].data.singer)}</span>`,
              delete:'删除'
          },
          handler:{
          ['cancel'](){
          ...
          ['delete'](){
            that.$store.commit('delFromPlayList', that.musicIndex)
          }
        }
      })
    }
