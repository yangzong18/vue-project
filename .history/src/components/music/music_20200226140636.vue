<template>
  <div class="music">
    <div class="music_content">
      ...
      <div class="music_ctrl circle" v-if="getCurrentMusic.name">
		<div class="d-audio-circle" :title="getCurrentMusic.name +'-'+getCurrentMusic.singer">
			<img class="avatar" style="background-size: cover;background-position: center center;" :src="getCurrentMusic.picurl+'?param=120y120)'">
			<div class="d-audio-range" style="border: 3px solid rgba(204, 188, 164, 0.36);"></div>
		</div>
        <div class="left_ctrl" v-show="false">
          <div class="music_detail_ctrl">
            <i
              class="playPause"
              :class="getAudioIsPlay ? 'icon-pause' : 'icon-play'"
              @click.stop="playPause()"
              title="播放/暂停 ctrl + space"
            ></i>
            <i class="playNext icon-nextdetail" title="下一曲 ctrl + right" @click.stop="playNext()"></i>
          </div>
		<div class="music_current_detail" v-show="false">
			<span class="music_c_name">{{getCurrentMusic.name }}</span>
			<span class="music_c_time">{{getCurrentMusic.singer}}</span>
		</div>
        </div>
		<div class="d-audio-blur-bg" v-show="false" style="background-size: cover;background-position: center center;" :style="{backgroundImage : 'url(' + getCurrentMusic.picurl + '?param=120y120)'}"></div>
      </div>
  ...
  methods: {
    ...
    keypress() {
      ...
      document.onkeydown = e => {
        if (e && e.ctrlKey && e.keyCode === 32) {
          // 同时按下 ctrl + 空格
          musicApi.playPause();
        }
        if (e && e.ctrlKey && e.keyCode === 39) {
          // 同时按下 ctrl + -->
          musicApi.playNextPrev(that, true);
        }
        if (e && e.ctrlKey && e.keyCode === 37) {
          // 同时按下 ctrl + <--
    ...
    selectAudioType(isclick) {
      let atype =
        localStorage.getItem("audioPlayType") ||
        store.getters.getAudioPlayType ||
        1;
...
.music {
  ...
  .music_content {
    ...
    .music_ctrl {
      ...
      overflow:hidden;
      animation:rerotateMusicLogo 8s linear infinite
    }
