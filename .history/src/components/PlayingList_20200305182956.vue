<template>
  <div id="playing-list">
    ...
    <div class="m-list">
      <ul >
        <li class="border-1px border-1px-after list-item">
          <div class="music-info">
            <p class="music-name">桥边姑娘</p>
            <p class="music-author">-花僮</p>
            <img
...
export default {
   ...
   methods:{
       ...
       showMenu(num){
        this.musicIndex = num;
        let that = this;
        this.$store.dispatch('nottifyActionSheet',{
            menus:{
                'title.noop':`${this.playList[num].name}<br/><span style="color:#666;font-size:12px;">${this.getSingerStr(this.playList[num].singer)`
            }
        })
       }
   },
