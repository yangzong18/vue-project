	...
	<div class="musiclist music_list_container">
		<scroll :data ="musiclist" class="music_l_content" :needPullUp="isNeedPull" @pullingUp="pullingUp">
			<div class="content">
				...
				<div class="music_list border-1px" v-if="musiclist" v-for="(list, index) in musiclist" :key="list.id" :data-musicid="list.id" :data-pic="list.album.picUrl" @click="clickPlayList(list.id, list.name, list.artists[0].picUrl, list.artists[0].name, getMusicDurationType(list.duration),index), musiclist">
