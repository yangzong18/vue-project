  ...
  export default {
    ...
    methods: {
      ...
      showMenu: function (num) {
        ...
        let that = this
        console.log(this.cd.songlist[num].songname)
        this.$store.dispatch('notifyActionSheet', {
          menus: {
            'title.noop': this.cd.songlist[num].songname + '<br/><span style="color:#666;font-size:12px;">' + this.cd.songlist[num].nickname+ '</span>',
            playAsNext: '下一首播放',
