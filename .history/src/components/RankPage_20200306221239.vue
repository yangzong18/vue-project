  ...
  methods:{
    ...
    showMenu(index){
      ...
      this.$store.dispatch('notifyActionSheet',{
          menus:{
              'title.noop':`${this.topListData.songlist[index].data.songorig}<br/><span style="color:#666;font-size:12px;">${this.$options.filters['capitalize'](this.topListData.songlist[index].data.singer)}</span>`,
              playAsNext: '下一首播放',
