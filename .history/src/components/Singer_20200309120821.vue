  ...
  methods: {
    ...
    showMenu(index) {
      ...
      this.$store.dispatch("notifyActionSheet", {
        menus: {
          "title.noop": `${this.singer.list[index].data.songorig}<br/><span style="color:#666;font-size:12px;">${this.$options.filters["singer"
          ](this.topListData.list[index].data.singer)}</span>`,
          playAsNext: "下一首播放",
        ...
        handler: {
          ...
          ["playAsNext"]() {
            that.$store.commit("addToPlayListAsNextPlay", {
              id: that.singer.list[that.menuedIndex].data.songid,
              mid: that.singer.list[that.menuedIndex].data.songmid,
              name: that.singer.list[that.menuedIndex].data.songorig,
              singer: that.singer.list[that.menuedIndex].data.singer,
              albummid: that.singer.list[that.menuedIndex].data.albummid
            });
          },
          ["addToPlayList"]() {
            that.$store.commit("addPlayList", {
              id: that.singer.songlist[that.menuedIndex].data.songid,
              mid: that.singer.songlist[that.menuedIndex].data.songmid,
              name: that.singer.songlist[that.menuedIndex].data.songorig,
              singer: that.singer.songlist[that.menuedIndex].data.singer,
              albummid: that.singer.songlist[that.menuedIndex].data.albummid
            });
          }
        }
      });
