        ...
        <li class="border-1px border-1px-after" v-for="(item,index) in singer.list" :key="index">
          ...
          <div class="music-info" @click="play(index)">
            ...
            <div class="music-singer">
              <span v-for="singername in item.musicData.singer" :keu>{{singername.name}}--</span>
...
export default {
  ...
  methods: {
    ...
    play(index) {
      let list = [];
      this.singer.songlist.forEach(item => {
        list.push({
          id: item.data.songid,
    ...
    showMenu(index) {
      ...
      this.$store.dispatch("notifyActionSheet", {
        menus: {
          "title.noop": `${this.singer.songlist[index].data.songorig}<br/><span style="color:#666;font-size:12px;">${this.$options.filters["singer"
          ](this.topListData.songlist[index].data.singer)}</span>`,
          playAsNext: "下一首播放",
  ...
  computed: {
    ...
    gradientcolor() {
      return (
        "-webkit-linear-gradient(top, rgba(" +
        this.r +
        "," +
        this.g +
        "," +
