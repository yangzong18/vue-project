...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      showMenu: function (num) {
        ...
        this.$store.dispatch('notifyActionSheet', {
          menus: {
            'title.noop': `${this.cd.songlist[num].songname}<br/><span style="color:#666;font-size:12px;">${this.$options.filters['singer'](this.cd.songlist[num].singer)}</span>`,
            playAsNext: '下一首播放',
          ...
          handler: {
            ...
            ['playAsNext'](){
              that.$store.commit('addToPlayListAsNextPlay', {
                id: that.cd.songlist[that.menuedIndex].songid,
                mid: that.cd.songlist[that.menuedIndex].songmid,
                name: that.cd.songlist[that.menuedIndex].songname,
                singer: that.cd.songlist[that.menuedIndex].nickname,
                albummid:that.cd.songlist[that.menuedIndex].albummid,
              })
            },
            ['addToPlayList'](){
              that.$store.commit('addToPlayList', {
                ...
                name: that.cd.songlist[that.menuedIndex].songname,
                singer: that.cd.songlist[that.menuedIndex].nickname,
                albumid:that.cd.songlist[that.menuedIndex].albummid,
              })
            }
          }
        })
    ...
    computed: {
      ...
      color: function () {
        if (this.cd !== null && this.cd.color) {
          let fixed = "00000" + this.cd.color.toString(16);
